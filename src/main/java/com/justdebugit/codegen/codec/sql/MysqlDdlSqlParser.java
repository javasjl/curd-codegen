    ...
    private static TableDefinition fromTableElement(MySqlCreateTableStatement statement) {
        ...
        List<TableFieldDefinition> fieldDefinitions = statement
                ...
                .map(item -> {
                    ...
                    String name = item.getDataType().getName();

                    fDefinition.setType(name);
                    // 判断tinyint长度是否为1 ，为 1 设置boolean类型
                    if (name.equalsIgnoreCase(MysqlTypeRepo.TINYINT_NAME)) {
                        SQLIntegerExpr sqlExpr = (SQLIntegerExpr) item.getDataType().getArguments().get(0);
                        Number number = sqlExpr.getNumber();
                        if (number.equals(1)) {
                            fDefinition.setType(MysqlTypeRepo.BOOLEAN_NAME);
                        }
                    }
